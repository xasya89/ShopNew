...
namespace ShopNew.Web.Infrastructure
{
    ...
    public class SessionStorageService : ISessionStorageService
    {
        ...
        {
            var data = JsonSerializer.Serialize(item);
            await _jsRuntime.InvokeVoidAsync("setSession", key, data);
        }

        public Task SetStringAsync(string key, string value)
        {
            _jsRuntime.InvokeAsync<string>("setSession", key, value);
            return Task.CompletedTask;
        }

        public async Task<T> GetAsync<T>(string key) where T : class
        {
            var data = await _jsRuntime.InvokeAsync<string>("getSession", key);
            if (string.IsNullOrEmpty(data))
            {
                return null!;
            }
        ...
        public async Task<string> GetStringAsync(string key)
        {
            return await _jsRuntime.InvokeAsync<string>("getSession", key);
        }

        public Task RemoveAsync(string key)
        {
            _jsRuntime.InvokeAsync<string>("removeSession", key);
            return Task.CompletedTask;
        }
    }
}
